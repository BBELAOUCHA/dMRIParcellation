...
#                 and tess connectivity matrix to reduce the memory and computation time.
...
class Mesh():  # class definition of mesh:-coordinates and tess connectivity
    ...
    def Remove_void_tracto(self, zero_tracto, nonzero_tracto):  # remove the void seeds from the parcellation
        ...
        self.vertices = self.vertices[nonzero_tracto, :] # remove the void seeds coordinates
...
class Regions_processing():  # functions on the regions
    ...
    def Add_zero_tracto_label(self, check_tracto, Regions):  # this function add the label of the detected
        ...
        for i in check_tracto.zero_tracto:  # add the label of void seeds
            All_seeds_label[i] = All_seeds_label[check_tracto.replacement[i]] # the label of void tractogram is the same as its nearest non void tracto
        return  All_seeds_label # add the label of all seeds including the void tractogram
    ...
    def Small_region(self, SizeRegion, Regions, Connectivity, integrated_regions):  # function used to merge small regions
        ...
        Z = np.where(SizeRegion <= integrated_regions)[0]  # get regions that have a cordinal less than integrated_regions
        X = np.zeros(len(Z))
        Q = len(Z)    # number of small regions
        while Q > 0:  # loop to merge small regions with bigger ones
            for i in range(len(Z)): # loop over the number of small regions
                insideregion, connected_regions = self.Neighbor_region(RegionsX, Z[i], Connectivity)
                sth = 0
       	        CX = Connectivity[np.array(insideregion), :] # get the tess connectivity mesh of only region Z[i]
                for j in range(len(connected_regions)): # loop over all regions
                    ind = np.where(RegionsX == connected_regions[j])[0]
                    C = CX[:, ind] # get the tess connectivity between region Un[j] and Z[i]
                    A = np.sum(C) # check if there is edges between Un[j] and Z[i]
                    if (A > sth) and (connected_regions[j] != 0):  # if there are edges and are different regions and region is different from mask
            ...
            for i in range(Q): # change the labeling  after the merging
            ...
            RegionsX = RegionX_
            Z = np.where(SizeRegion <= integrated_regions)[0] # get the regions with small size
            X = np.zeros(len(Z))
    ...
    def Excluded_label(self, excluded, Labelnonexcluded, label_orig):  # function used to add the label of the # excluded seeds
