...
class Parcellation_data():# class of operations on the tractogram images
    ...
    def Read_tractograms(self,V):
        ...
        for i in range(len(V[:,0])):
            ...
            self.tractograms.append(A.reshape(-1))
...
print "Tracto prefix Name:",Arg.tract_name  # begining of tracto name "name_x_y_z.nii.gz"
...
nbr_remaining_i=int(Arg.iteration)# number of growing iteration
...
if  os.path.exists(Arg.excluded):
...
for AT in ATLAS:
    ...
    save_results_path=str(Arg.save)+"/"+str(AT)
    ...
    savetxt(save_results_path+'/RealSurface/Labels.txt',RX)
    ...
    for i in range(len(NBR)):
            ...
            A=Parc.Correlation_Matrix[index,:]
            A=A[:,index]
            Si[i] = np.mean(A)
    savetxt(save_results_path+'/MCCP_end.txt',Si)
